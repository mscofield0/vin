...
fn form_vin_context_struct(name: &Ident, fields: &Fields, other_attrs: &Vec<&Attribute>) -> TokenStream2 {
    let context_struct_name = form_context_struct_name(name);
    ...
    let mailbox_struct = quote! {
        ...
        impl Default for #mailbox_struct_name {
            fn default() -> Self {
                ...
                #(erased_mailboxes.insert(::core::any::TypeId::of::<#msg_names>(), Box::new(#tx_wrapper_name(#msg_short_names.0.clone())) as ::vin::vin_core::BoxedErasedTx);)*
